...
plot <- ggplot() +
  ...
  labs(
    x = "Nuclear fraction",
    y = "UMI sum (log10)",
    title = sprintf("DropletQC (%s)", sample_name),
    subtitle = sprintf("%s barcodes", format(sum(nrow(cell_stats), big.mark = ",")))
  ) +
  guides(colour = "none") +
  theme_bw()
...
```{r plot_dropletqc_vireo_assigned_donor, echo=FALSE}
plot <- ggplot() +
  geom_point(
    mapping = aes(x = nuclear_fraction, y = log10(sum)),
...
```{r plot_dropletqc_vireo_assigned_doublet, echo=FALSE}
plot <- ggplot() +
  geom_point(
    mapping = aes(x = nuclear_fraction, y = log10(sum)),
...
```{r plot_dropletqc_vireo_unassigned, echo=FALSE}
plot <- ggplot() +
  geom_point(
    mapping = aes(x = nuclear_fraction, y = log10(sum)),
